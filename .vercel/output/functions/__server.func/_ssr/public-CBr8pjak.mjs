...
import { t as createOperatorSession } from "./operator-session.server-CcB7by9_.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/public-CBr8pjak.js
...
var logoutOperator = createServerFn({ method: "POST" }).handler(logoutOperator_createServerFn_handler, async () => {
	const { clearOperatorSession } = await import("./operator-session.server-CcB7by9_.mjs").then((n) => n.r).then((n) => n.r);
	clearOperatorSession();
	return { ok: true };
});
//#endregion
export { getPublicHome_createServerFn_handler, loginOperator_createServerFn_handler, logoutOperator_createServerFn_handler };
